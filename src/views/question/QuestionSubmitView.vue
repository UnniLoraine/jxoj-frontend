    ...
    >
      <template #judgeInfo="{ record }">
        <a-space wrap>
          ...
          <a-tag color="cyan">
            内存：
            {{ record.judgeInfo.memory === null ? 0 : record.judgeInfo.memory }}
          </a-tag>
          <a-tag color="blue">
            时间：
            {{ record.judgeInfo.time === null ? 0 : record.judgeInfo.time }}
          </a-tag>
        </a-space>
        <!--        {{ record.judgeInfo }}-->
      </template>
      ...
      <template #judgeStatus="{ record }">
        <template v-if="record.status === 0">
          <a-tag bordered>等待中</a-tag>
        </template>
        <template v-else-if="record.status === 1">
          <a-tag bordered color="orangered"> 判题中</a-tag>
        </template>
        <template v-else-if="record.status === 2">
          <a-tag bordered color="green">判题完成</a-tag>
        </template>
        <template v-else>
          <a-tag bordered color="red">失败</a-tag>
        </template>
      </template>
    </a-table>
  </div>
